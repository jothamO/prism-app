...
import { Search, MoreHorizontal, ArrowUpDown, UserPlus, Ban, CheckCircle, Clock } from "lucide-react";
...
const MOCK_USERS: User[] = [
  { id: "1", name: "Chidi Okeke", email: "chidi@example.com", role: "user", status: "active", lastActive: "2 mins ago" },
  { id: "2", name: "Amaka Igwe", email: "amaka@example.com", role: "admin", status: "active", lastActive: "1 hour ago" },
  { id: "3", name: "Tunde Bakare", email: "tunde@example.com", role: "user", status: "suspended", lastActive: "5 days ago" },
  { id: "4", name: "Bisi Adebayo", email: "bisi@example.com", role: "support", status: "active", lastActive: "Just now" },
  { id: "5", name: "Emeka Okafor", email: "emeka@example.com", role: "user", status: "pending", lastActive: "Never" },
];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        ...
        <div className="p-4 border-t border-border flex items-center justify-between text-sm text-muted-foreground">
          <span>Showing {table.getRowModel().rows.length} of {MOCK_USERS.length} users</span>
          <div className="flex items-center gap-2">
            <button className="px-3 py-1 border border-border rounded hover:bg-accent disabled:opacity-50" onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()}>Previous</button>
            <button className="px-3 py-1 border border-border rounded hover:bg-accent disabled:opacity-50" onClick={() => table.nextPage()} disabled={!table.getCanNextPage()}>Next</button>
          </div>
        </div>
      </div>
    </div>
