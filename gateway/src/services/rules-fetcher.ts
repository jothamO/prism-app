...
async function getActiveRulesForDate(ruleType: string | undefined, asOfDate: Date): Promise<TaxRule[]> {
  try {
    ...
    if (error) {
      logger.error('Error fetching rules for date:', error);
      return [];
    }
    
    return data || [];
  } catch (error) {
    logger.error('Failed to fetch rules for date:', error);
    return [];
  }
}
...
export async function getUpcomingRules(ruleType?: string): Promise<TaxRule[]> {
  try {
    ...
    if (error) {
      logger.error('Error fetching upcoming rules:', error);
      return [];
    }
    
    return data || [];
  } catch (error) {
    logger.error('Failed to fetch upcoming rules:', error);
    return [];
  }
}
